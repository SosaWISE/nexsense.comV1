app.controller("SellForNexsensePage", ['$scope', '$http', '$location', 'FormValidation', 'Site', 'Messaging', function($scope, $http, $location, validation, $site, $msg) {
	...
	$scope.submitApplication = function() {
		...
		if (!$scope.appForm.$valid) {
			$msg.displayMessage("There were some problems with your application.  Please check your information and try again.", true);
			return;
		}
		...
		$http({
			method: 'POST',
			url: 'ng/Contact/submit_application',
			data: postData
		}).
		success(function(data, status, headers, config) {
			console.log("success");
			console.log(data);
			$msg.displayMessage("Your application was successfully sent to Nexsense.  Thanks for applying.");
			$scope.isSubmitting = false;
			$scope.resetvars();
			...
			console.log("ERROR");
			console.log(data);
			$msg.displayMessage("There was a problem submitting your application.  Please ensure all fields are complete.", true);
			$scope.isSubmitting = false;
		});
	}
